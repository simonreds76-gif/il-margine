#!/usr/bin/env python3
"""
Download match-level team shooting stats enriched with xG data.

Uses Understat JSON API for match-level xG (same StatsBomb/Opta data
that FBref displays) and merges with existing Football-Data.co.uk
historical files for actual shot counts.

Output:
  data/team-shots/fbref/all-fbref-matches.csv
...
OUTPUT_DIR = ROOT / "data" / "team-shots" / "fbref"
OUTPUT_FILE = OUTPUT_DIR / "all-fbref-matches.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download FBref/Understat shooting data")
