...
It will:
  ...
  5. Apply fractional Kelly staking
...
# ── Poisson maths (duplicated to keep standalone) ─────────────────
...
def format_shortlist(
    ...
    lines.append("  Staking: 0.5u (12-15% edge) | 1u (15-20%) | 1.5u (20-25%) | 2u (25%+)")
    lines.append("=" * 78)
    return "\n".join(lines)
...
def main() -> None:
    ...
    parser.add_argument("--bankroll", type=float, default=DEFAULT_BANKROLL,
                        help=f"Bankroll size in units (default {DEFAULT_BANKROLL})")
    ...
    if all_value_bets:
        ...
        fields = list(all_value_bets[0].keys())
