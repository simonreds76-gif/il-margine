#!/usr/bin/env python3
"""
Fit Platt scaling calibration on held-out historical corner predictions.
...
JSON and loaded at runtime by matchday-shortlist.py.

Usage:
  python scripts/corners-fit-calibration.py
  python scripts/corners-fit-calibration.py --fit-before 2023-08-01
"""
...
DEFAULT_PARAMS_OUT  = ROOT / "data" / "corners-ou" / "corners-calibration-params.json"
DEFAULT_DIAG_OUT    = ROOT / "data" / "corners-ou" / "corners-calibration-diagnostics.txt"
...
def platt_fit(
    ...
    the identity transform (a=1, b=0).  Normalises gradients by n to make
    lr scale-invariant to dataset size.
    ...
    for _ in range(steps):
        ...
        for xi, yi in zip(xs, actuals):
            ...
            da += err * xi
            db += err
        # normalise + L2 toward identity (a=1, b=0)
        a += lr * (da / n - l2 * (a - 1.0))
        b += lr * (db / n - l2 * b)
    return a, b
...
def reliability_table(
    ...
    """Returns list of (lo, hi, n, pred_mean, actual_rate)."""
...
def ece(table: List[Tuple[float, float, int, float, float]]) -> float:
    """Expected Calibration Error: weighted mean absolute deviation per bin."""
...
def load_and_split(
    ...
    Rows with no actual_total (upcoming fixtures) are excluded automatically.
    """
    fit: List[dict] = []
    val: List[dict] = []
    with open(path, encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            ...
            if _pf(row.get("actual_total", "")) < 0.5:
                continue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fit per-line Platt calibration for corners O/U model"
    )
    ...
    if len(fit_rows) < 200:
        print(
            "ERROR: fewer than 200 fit rows - run corners-ou-model.py first "
            "or widen --fit-before"
        )
        return
