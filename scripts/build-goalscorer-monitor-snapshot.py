...
def compute_shadow_summary(rows: list[dict[str, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "losses": losses,
        "voids": voids,
        "roi": (pnl_units / len(settled) * 100) if settled else 0,
        "pnl_units": pnl_units,
    }
...
def penalty_component_meta(row: dict[str, str] | None) -> dict[str, str] | None:
    ...
    detail_bits.append(f"λpen {penalty_lambda:.3f}")
    detail_bits.append(f"λopen {non_pen_lambda:.3f}")

    compact_bits = [
        f"{penalty_role_label(role)} pen" if role != "none" else "Pen angle",
        f"{(penalty_share * 100):.0f}% share",
        f"λ {penalty_lambda:.3f}",
    ]
...
def enrich_penalty_review_row_with_match_result(row: dict[str, Any]) -> dict[str, Any]:
    ...
    next_row["active_on_pitch_at_penalty"] = row.get("active_on_pitch_at_penalty") or describe_player_at_penalty(active_player, minute, roster_known)
...
def read_penalty_review_rows(config: dict[str, str]) -> tuple[list[dict[str, Any]], str | None]:
    ...
    for row in enriched:
        rows.append({
            ...
            "date": row.get("date"), "league": row.get("league"), "review_source": row.get("review_source"),
            "review_priority": row.get("review_priority"), "review_type": row.get("review_type"), "match": row.get("match"),
            "team": row.get("team"), "opponent": row.get("opponent"), "actual_taker": row.get("actual_taker"),
            "actual_role_pre_match": row.get("actual_role_pre_match"), "penalties_attempted": parse_int(row.get("penalties_attempted")),
            "penalties_scored": parse_int(row.get("penalties_scored")), "distinct_takers_in_match": parse_int(row.get("distinct_takers_in_match")),
            "minute": row.get("minute"), "event_type": row.get("event_type"), "event_result": row.get("event_result"),
            "primary_pre_match": row.get("primary_pre_match"), "secondary_pre_match": row.get("secondary_pre_match"), "tertiary_pre_match": row.get("tertiary_pre_match"),
            "primary_lineup_status": row.get("primary_lineup_status"), "secondary_lineup_status": row.get("secondary_lineup_status"), "tertiary_lineup_status": row.get("tertiary_lineup_status"),
            "active_taker_pre_match": row.get("active_taker_pre_match"), "active_slot_pre_match": row.get("active_slot_pre_match"), "team_lineup_status": row.get("team_lineup_status"),
            "penalty_transfer_pre_match": parse_int(row.get("penalty_transfer_pre_match")), "inherited_from_pre_match": row.get("inherited_from_pre_match"), "transfer_level_pre_match": row.get("transfer_level_pre_match"),
            "editorial_note": row.get("editorial_note"), "context_generated_at": row.get("context_generated_at"), "context_source_path": row.get("context_source_path"),
            "primary_on_pitch_at_penalty": row.get("primary_on_pitch_at_penalty"), "active_on_pitch_at_penalty": row.get("active_on_pitch_at_penalty"), "actual_taker_on_pitch_at_penalty": row.get("actual_taker_on_pitch_at_penalty"),
        })
