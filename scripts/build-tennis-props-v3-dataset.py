...
def load_matches(start_year: int, end_year: int) -> list[dict[str, str]]:
    rows: list[dict[str, str]] = []
    for tour in ("ATP", "WTA"):
        for year in range(start_year, end_year + 1):
            path = SACKMANN_DIR / f"{tour.lower()}_matches_{year}.csv"
            if not path.exists():
                continue
            with path.open("r", encoding="utf-8-sig", newline="") as handle:
                for raw in csv.DictReader(handle):
                    ...
                    if match_date is None or level not in main_levels(tour) or surface == "Unknown":
                        continue
...
def observation(row: dict[str, str], prefix: str, opponent_prefix: str) -> Observation:
    return Observation(
        ...
        opponent_aces=parse_int(row.get(f"{opponent_prefix}_ace")),
...
def player_features(
    ...
    features: dict[str, float] = {
        "ace_rate_blend": blend_rate(windows, "aces", "svpt", prior_ace, ace_weight),
        ...
        "aces_allowed_blend": blend_rate(windows, "opponent_aces", "opponent_svpt", prior_ace, 400.0),
        "svpt_per_svg_blend": blend_rate(windows, "svpt", "svgms", 6.35, 60.0),
    }
    for name, stats in windows.items():
        features[f"{name}_matches"] = stats.get("matches", 0.0)
        features[f"{name}_svpt"] = stats.get("svpt", 0.0)
        features[f"{name}_ace_rate"] = safe_rate(stats, "aces", "svpt", prior_ace)
        ...
        features[f"{name}_aces_allowed_rate"] = safe_rate(stats, "opponent_aces", "opponent_svpt", prior_ace)
...
def build_dataset(rows: list[dict[str, str]], output_start_year: int) -> list[dict[str, object]]:
    player_history: dict[tuple[str, str, str], list[Observation]] = defaultdict(list)
    surface_stats: dict[tuple[str, str], dict[str, float]] = defaultdict(blank_stats)
    venue_stats: dict[tuple[str, str, str], dict[str, float]] = defaultdict(blank_stats)
    workload_stats: dict[tuple[str, str, str, int], dict[str, float]] = defaultdict(blank_stats)
    ...
    for date_key in sorted(rows_by_date):
        ...
        for row in day_rows:
            if int(date_key[:4]) < output_start_year:
                continue
            common = {
        ...
        for row in day_rows:
            ...
            player_history[(tour, str(row.get("winner_id") or ""), surface)].append(winner_obs)
            player_history[(tour, str(row.get("loser_id") or ""), surface)].append(loser_obs)
            surface_key = (tour, surface)
            venue_key = (tour, norm_tournament(row.get("tourney_name")), surface)
...
def main() -> int:
    ...
    rows = build_dataset(matches, args.output_start_year)
