...
def load_aliases(rows: list[dict[str, str]]) -> dict[tuple[str, str], str]:
...
def build_feature_row(
    ...
    baseline_names: dict[str, set[str]] | None = None,
    ...
    names = baseline_names or baseline_names_by_tour(baselines)
    player_lookup = str(row.get("player_baseline_name") or "").strip()
    opponent_lookup = str(row.get("opponent_baseline_name") or "").strip()
    if not player_lookup:
        player_lookup = resolve_baseline_name(
            tour=tour,
            value=row.get("player"),
            available_names=names,
            aliases=aliases,
        ).name
    if not opponent_lookup:
        opponent_lookup = resolve_baseline_name(
            tour=tour,
            value=row.get("opponent"),
            available_names=names,
            aliases=aliases,
        ).name
    player_rows = baselines.get((tour, player_lookup, surface), {})
    opponent_rows = baselines.get((tour, opponent_lookup, surface), {})
...
def main() -> int:
    ...
    baseline_names = baseline_names_by_tour(baselines)
    aliases = load_aliases(read_csv(args.aliases))
    ...
    for row in board_rows:
        ...
        out_row = dict(row)
        out_row.update({
            "player_l12m_matches": f"{number(feature_row.get('player_l12m_matches')):.0f}",
            "player_l12m_svpt_sample": f"{number(feature_row.get('player_l12m_svpt')):.0f}",
            "player_l24m_matches": f"{number(feature_row.get('player_l24m_matches')):.0f}",
            "player_l24m_svpt_sample": f"{number(feature_row.get('player_l24m_svpt')):.0f}",
            "player_career4y_matches": f"{number(feature_row.get('player_career4y_matches')):.0f}",
            "player_career4y_svpt_sample": f"{number(feature_row.get('player_career4y_svpt')):.0f}",
