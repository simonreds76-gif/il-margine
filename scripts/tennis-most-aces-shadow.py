...
FIELDS = [
    ...
    "overround", "value_player1_pct", "value_draw_pct", "value_player2_pct",
...
def board_index(rows: list[dict[str, str]]) -> dict[tuple[str, tuple[str, str]], dict[str, str]]:
    return {
        (row.get("date", ""), pair_key(row.get("player1"), row.get("player2"))): row
        for row in rows
    }
...
def register(board_rows: list[dict[str, str]], captures: list[dict[str, str]], ledger: list[dict[str, str]]) -> int:
    ...
    for capture in captures:
        board = index.get((capture.get("date", ""), pair_key(capture.get("player1"), capture.get("player2"))))
        if not board:
            continue
...
def update_closes(ledger: list[dict[str, str]], history: list[dict[str, str]]) -> int:
    ...
    for row in history:
        grouped[observation_id(row)].append(row)
    updated = 0
    for row in ledger:
        ...
        if not registration or not start:
            continue
        candidates = []
        for capture in grouped.get(row.get("observation_id", ""), []):
...
def main() -> int:
    ...
    added = register(read_csv(Path(args.board)), captures, ledger)
