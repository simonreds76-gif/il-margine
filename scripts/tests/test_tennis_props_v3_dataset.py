...
class TennisPropsV3DatasetTests(unittest.TestCase):
    ...
    def test_a3_activity_uses_prior_lower_level_matches_without_same_day_leakage(
        self,
    ) -> None:
        lower = match_row("2023-01-01", "1", "2", 1)
        lower.update({
            "_source": "qual_chall",
            "_target": "no",
            "winner_rank": "300",
        })
        current = match_row("2023-04-11", "1", "3", 2)
        current["winner_rank"] = "200"

        output = V3.build_dataset(
            [lower, current],
            output_start_year=2023,
            include_a3_features=True,
        )
        player = next(row for row in output if row["player_id"] == "1")

        self.assertEqual(player["player_activity_days_since_match_all"], 100)
        self.assertEqual(player["player_activity_matches_l365d_all"], 1)
        self.assertEqual(player["player_activity_lower_matches_l365d"], 1)
        self.assertEqual(player["player_activity_lower_share_l365d"], 1.0)
        self.assertEqual(player["player_activity_rank_90d_known"], 1.0)
        self.assertAlmostEqual(
            player["player_activity_rank_log_change_90d"],
            math.log1p(200) - math.log1p(300),
        )

    def test_a3_same_date_activity_is_frozen(self) -> None:
        rows = [
            match_row("2023-01-01", "1", "2", 1),
            match_row("2023-01-01", "1", "3", 2),
        ]
        ...
        player_rows = [row for row in output if row["player_id"] == "1"]
        self.assertEqual(len(player_rows), 2)
        self.assertTrue(all(
            row["player_activity_matches_l90d_all"] == 0
            for row in player_rows
        ))

    def test_live_feature_state_reproduces_backtest_prematch_features(self) -> None:
        prior = match_row("2023-01-01", "1", "2", 1)
        target = match_row("2023-02-01", "1", "3", 2)
        expected = next(
            row for row in V3.build_dataset(
                [prior, target],
                output_start_year=2023,
                include_a3_features=True,
            )
            if row["date"] == "2023-02-01" and row["player_id"] == "1"
        )
        state = V3.build_feature_state(
            [prior, target],
            as_of=V3.date.fromisoformat("2023-02-01"),
        )
        actual = V3.side_row(
            target,
            prefix="w",
        ...
        for field in compared:
            self.assertAlmostEqual(float(actual[field]), float(expected[field]))
