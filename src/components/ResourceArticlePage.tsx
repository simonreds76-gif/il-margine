...
export default function ResourceArticlePage({ eyebrow, title, description, canonicalPath, datePublished, dateModified, toc, children, takeaway }: Props) {
  ...
  return <div className="guide-page">
    ...
    <header className="guide-hero"><div className="guide-container">
      ...
      <div className="guide-hero-title"><ToolEmblem name={emblemForHref(canonicalPath)} /><div><div className="guide-label">{eyebrow}</div><h1>{title}</h1><p className="guide-deck">{description}</p><div className="guide-meta"><span>By Il Margine</span><time dateTime={modified}>Updated {new Intl.DateTimeFormat("en-GB", { day: "numeric", month: "long", year: "numeric" }).format(new Date(`${modified}T12:00:00Z`))}</time>{current && <span>{current.minRead} min read</span>}</div></div></div>
    </div></header>
    <main className="guide-container guide-layout"><ResourceContentsNav items={toc} /><article className="guide-copy">
      ...
      {children}
      <section className="guide-related" aria-labelledby="related-guides"><div className="guide-label">Put it into practice</div><h2 id="related-guides">Keep learning</h2><div className="guide-related-grid"><Link href="/tools" prefetch={false}><ToolEmblem name="tools" /><h3>Find the right betting tool</h3></Link>{related.map(r => <Link key={r.href} href={r.href}><ToolEmblem name={emblemForHref(r.href)} /><h3>{r.title}</h3></Link>)}</div></section>
    </article></main><Footer />
  </div>;
}
