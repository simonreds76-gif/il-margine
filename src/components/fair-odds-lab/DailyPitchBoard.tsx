...
function MatchPitch({ fixture: f, now, snapshot, view, selected, setSelected }: { fixture: BoardFixture; now: number; snapshot: string | null; view: string; selected: PlayerSelection | null; setSelected: (value: {side: number; id: string} | null) => void }) {
  ...
  return <details id={`match-${f.id}`} className="ip-match" aria-label={`${f.teams[0].name} versus ${f.teams[1].name}`}>
    ...
    <span className="ip-expand-hint"><span className="ip-when-closed">Show lineup &amp; odds</span><span className="ip-when-open">Hide lineup &amp; odds</span><span aria-hidden="true">⌄</span></span>
    </summary>
...
export function DailyPitchBoard({ initial, boardUrl, asOf, preview = false }: { initial: DailyBoard; boardUrl: string; asOf: number; preview?: boolean }) {
  ...
  return <div id="im-pitch-lab"><div className="ip-main"><div className="ip-head"><div><div className="ip-eyebrow">THE GOALSCORER WORKBENCH</div><h1>Fair Odds <span>Lab</span></h1><p className="ip-sub">Read the XI. Compare the price. Find your edge.</p></div><div className="ip-headlinks"><span className="ip-beta">Free beta</span><a href="#lab-hits">Latest hits ↗</a></div></div>
    ...
    <details id="lab-signals" className="ip-shortlist" aria-label="Suggested signals">
      ...
      </article>)}</div><p className="ip-shortlist-note">These are model-based comparisons, not guaranteed winners. Expected starters can change. Eligible comparisons recorded with confirmed lineups are tracked before kickoff; winning results can appear in Latest hits, including Super Sub wins.</p></> : <p className="ip-shortlist-empty">{!upcoming.length ? "No pre-match players in this view. Choose another date to see upcoming opportunities." : staleOdds > 0 || old ? "Waiting for refreshed Bet365 odds. Older prices remain visible in Matches & odds, but are not current signals. This does not mean there is no value." : checkedPlayers === 0 ? "We do not yet have enough usable prices and player data to check for value in this view." : "No players currently pass the price and lineup checks. A higher bookmaker price alone is not enough when player data or the starting lineup is incomplete."}</p>}
    </details>
    <details id="lab-matches" className="ip-matches-panel"><summary className="ip-panel-summary"><span><strong>Matches &amp; odds</strong><small>Open a match to view both teams, lineups and prices</small></span><span>{fixtures.length} matches <span aria-hidden="true">⌄</span></span></summary>
