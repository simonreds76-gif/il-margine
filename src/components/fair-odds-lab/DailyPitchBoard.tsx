...
export function DailyPitchBoard({ initial, boardUrl, asOf, preview = false, highlights }: { initial: DailyBoard; boardUrl: string; asOf: number; preview?: boolean; highlights?: ReactNode }) {
  ...
  return <div id="im-pitch-lab"><div className="ip-main"><PageHomeLink /><div className="ip-head"><div><div className="ip-eyebrow">INDEPENDENT GOALSCORER ANALYSIS</div><h1>Fair Odds <span>Lab</span></h1><p className="ip-sub">Explore the lineups. Compare our fair odds with Bet365. Open a player for the detail.</p></div><div className="ip-headlinks"><span className="ip-beta">Free beta</span><a href="#lab-hits">Latest hits ↗</a></div></div>
    ...
    {highlights}
    <PlayerLabelGuide />
    {(old || refreshFailed) && <p className="ip-pennews" role="status">{old && board.fixtures.length === 0 ? "The lineup feed has not supplied a recent update. We cannot confirm current match or price coverage yet." : old ? "The latest update is overdue. You can still browse the last prices, but they won’t appear in the value shortlist until checked again." : "We couldn’t load the latest update. Showing the last available prices."}</p>}
    <details id="lab-signals" className="ip-shortlist" aria-label="Price comparisons">
      ...
      {upcoming.length > 0 && <div className="ip-signal-health" role="status"><span>{checkedPlayers} players with usable comparisons</span><span>{staleOdds} odds awaiting a fresh check</span><span>{missingOdds} players without Bet365 odds</span></div>}
    ...
    <details id="lab-matches" className="ip-matches-panel" open={fixtures.length === 0 ? true : undefined}><summary className="ip-panel-summary"><span><strong>Matches &amp; odds</strong><small>Open a match to view both teams, lineups and prices</small></span><span className="ip-summary-actions"><span>{fixtures.length} matches</span><DisclosureCue /></span></summary>
