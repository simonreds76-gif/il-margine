    ...
    {view === "pitch" ? <div className="ip-board">{f.teams.map((team, teamIndex) => {
      ...
      return <section key={team.name} className={`ip-club ${side === teamIndex ? "ip-active" : ""}`} style={{ "--ip-kit": team.primaryColor?.match(/^#[0-9a-f]{6}$/i) ? team.primaryColor : teamIndex ? "#efc2d1" : "#c6e7ff", "--ip-kit-ink": "#ffffff" } as CSSProperties}>
        ...
        <div className={`ip-field ${validLines ? "" : "ip-ungrouped"}`}><div className="ip-lines" aria-hidden="true"><div className="ip-center" /><div className="ip-area top" /><div className="ip-area bottom" /></div>
          {lines.map((line, i) => <div key={i} className="ip-row">{line.map(p => { const c = comparison(p, f, now, snapshot); return <button key={p.id} type="button" className="ip-player" aria-label={`${p.name}, our fair odds ${decimal(c.fair)}, Bet365 odds ${decimal(p.bookmakerOdds)}${!c.fresh ? ", quote missing or stale" : ""}`} aria-pressed={selected?.side === teamIndex && selected.id === p.id} aria-controls={detailId} onClick={() => setSelected({ side: teamIndex, id: p.id })}><Portrait player={p} />{p.penaltyActive && <span className="ip-pen">PEN</span>}<span className="ip-name">{p.name}</span><span className={`ip-price ${!c.fresh ? "ip-aged" : ""}`}><span className="ip-fair" title={p.pricingReason || "Our estimated fair odds"}><span className="ip-price-label">Our fair odds</span><strong>{c.fair === null ? "Unpriced" : decimal(c.fair)}{p.pricingStatus === "limited_data" ? "*" : ""}</strong></span><span className="ip-book" title={p.bookmakerReason || "Bet365 decimal odds"}><span className="ip-price-label">Bet365</span><strong>{p.bookmakerOdds === null ? "No quote" : decimal(p.bookmakerOdds)}</strong></span></span><span className={`ip-gap ${c.gap !== null && c.gap < 0 ? "minus" : ""}`}>{c.gap === null ? p.pricingStatus === "limited_data" ? "Estimate*" : started ? "Match started" : p.bookmakerOdds && !c.fresh ? "Old quote" : "Not compared" : priceVerdict(c.gap)}</span></button>; })}</div>)}
