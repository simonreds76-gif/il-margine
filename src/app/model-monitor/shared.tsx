...
function cp850Byte(value: string): number | null {
  const codePoint = value.codePointAt(0);
  if (codePoint === undefined) return null;
  if (codePoint < 0x80) return codePoint;
  const index = CP850_HIGH.indexOf(value);
  return index < 0 ? null : index + 0x80;
}

function repairCp850Utf8(value: string): string {
  const characters = Array.from(value);
  let repaired = "";
  for (let index = 0; index < characters.length;) {
    const lead = cp850Byte(characters[index]);
    const length = lead !== null && lead >= 0xc2 && lead <= 0xdf
      ? 2
      : lead !== null && lead >= 0xe0 && lead <= 0xef
        ? 3
        : lead !== null && lead >= 0xf0 && lead <= 0xf4
          ? 4
          : 0;
    if (length > 0 && index + length <= characters.length) {
      const bytes = characters.slice(index, index + length).map(cp850Byte);
      if (bytes.every((byte, byteIndex) => byte !== null && (byteIndex === 0 || (byte >= 0x80 && byte <= 0xbf)))) {
...
function repairMojibake(value: string): string {
  ...
  const normalized = repairCp850Utf8(value.normalize("NFC"));
