...
const CALCULATOR_FAQS = [
  ...
  },
  {
    ...
    answer:
      "The Returns tab falls back to the last recorded baseline numbers and says so on screen. The Kelly, Fair odds and Closing line tools use only the numbers you enter, so they work regardless.",
  },
] as const;
...
function useRecordSummary(): RecordSummary {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const sum = (rows: CategoryStats[], key: keyof CategoryStats): number =>
        rows.reduce((total, row) => total + (Number(row[key]) || 0), 0);

      const propsLiveBets = sum(propsRows, "total_bets");
      const tennisLiveBets = sum(tennisRows, "total_bets");
      const propsLiveStake = sum(propsRows, "total_stake") || propsLiveBets;
      const tennisLiveStake = sum(tennisRows, "total_stake") || tennisLiveBets;

      const totalBets =
        BASELINE_STATS.props.total_bets + BASELINE_STATS.tennis.total_bets + propsLiveBets + tennisLiveBets;
      const wins = BASELINE_STATS.overall.wins + sum(propsRows, "wins") + sum(tennisRows, "wins");
      const losses = BASELINE_STATS.overall.losses + sum(propsRows, "losses") + sum(tennisRows, "losses");
      const totalProfit =
        BASELINE_STATS.overall.total_profit + sum(propsRows, "total_profit") + sum(tennisRows, "total_profit");
      ...
      track("calculator_data_loaded", {
        roi_used: calculateROI(totalProfit, totalStake || 1),
        bets_count: totalBets,
      });
    } catch {
