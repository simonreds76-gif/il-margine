# Full debrief for Codex: H2H + Advanced stats (from scratch)

**Use this doc if you’ve never used Codex (or similar) before.** It explains the project, the data, and exactly what to build so another AI can implement it.
...
So we are **predicting** match outcomes and totals; then we compare our numbers to the bookmaker. The goal of the work described here is to **improve our match-winner (ML) predictions** by using two things we don’t use yet: **head-to-head (H2H)** and **advanced serve/return stats**.
...
Put the SQL in a file under `docs/`, e.g. `docs/supabase-player-h2h.sql` and `docs/supabase-player-advanced-stats.sql`, and tell the user to run them in the Supabase SQL editor.
...
# Part 6 — How to use this with Codex (first time)

## Do not write to the repo — paste code in chat only

**Tell Codex (or the AI) explicitly:** Do not write, edit, or create any files in the repository. Do not push, commit, or change the repo on disk or via GitHub. Instead, output all code (SQL, Python, and any edits to existing files) as **pasteable blocks in the chat** (e.g. markdown code blocks with file paths). The user will review and copy the code into the project themselves. This way nothing is written over the repo without the user’s control.

You can say for example: “Follow the debrief and implement everything, but do not write any files. Put all code in your reply so I can paste it in yourself.”

---

## Steps to use Codex

1. **Open the project** in your editor (e.g. Cursor) so Codex can see the repo (or give it access to the repo as needed).
2. **Give Codex this document** (or the link to it). Say something like: “Implement the H2H and advanced stats pipeline exactly as described in docs/DEBRIEF-H2H-AND-ADVANCED-STATS.md. Do Part 4 step by step: create the two SQL files, then the ID mapping and two Python scripts, then the two integrations in oncourt-compute-fair-odds.py. **Do not write or edit any files — output all code in the chat so I can paste it in myself.**”
3. **Point to key files:**  
   - Fair-odds: `scripts/oncourt-compute-fair-odds.py`  
   - OnCourt stats (reference only): `scripts/oncourt-compute-player-stats.py`  
   - Data: `data/sackmann/`, `tml-data/`, `data/oncourt/`
4. **Environment:** Scripts that write to Supabase need `.env.local` with `NEXT_PUBLIC_SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY`. Tell Codex (and the user) that these scripts should load env from the project root (same as other scripts in the repo).
5. **Run order:** User will run (1) SQL in Supabase, (2) H2H script, (3) Advanced-stats script, (4) fair-odds as usual. So the new scripts should be runnable like:  
   `python scripts/sackmann-compute-h2h.py [--dry-run]`  
   `python scripts/sackmann-compute-advanced-stats.py [--dry-run]`  
   (or similar names under `scripts/`).
